...
const ChildTabContent = ({ child }: { child: Child }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                // Fetch direct matches (available kindergartens)
                ...
                // Fetch assigned matches (active exchanges)
                const assignedMatchesRes = await fetch(`http://localhost:3001/matching/child/${child.id}/groups`, {
                    headers: {
                        'Authorization': `Bearer ${token}`
                    }
                });

                if (assignedMatchesRes.ok) {
                    setAssignedMatches(await assignedMatchesRes.json());
                }
    ...
    if (loading) return <div className="text-center p-4 text-color-text-muted">Učitavanje podataka o detetu...</div>;
        ...
        <div className="space-y-6 animate-fade-in-up">
            <div className="glass-card p-6 rounded-2xl border border-white/50">
                ...
                {child.current_kindergarten ? (
                    <div className="bg-white/50 p-4 rounded-xl">
                        <p className="font-semibold text-lg">{child.current_kindergarten.name}</p>
                        <p className="text-color-text-muted">{child.current_kindergarten.address}</p>
                    </div>
                ) : (
                    ...
                    <div className="grid gap-3">
                        {assignedMatches.map(group => {
                            ...
                            const otherParticipants = group.participants
                                .filter((p: any) => p.child.id !== child.id)
                                .map((p: any) => p.child);

                            return (
                                <div key={group.id} className="bg-white p-4 rounded-xl shadow-sm border border-teal-100">
                                    <div className="flex justify-between items-start mb-3">
                                        <div>
                                            <p className="font-bold text-lg mb-1">
                                                {isActive ? 'Kontakt razmena aktivna!' : 'Potrebno prihvatanje'}
                                            </p>
                                            <p className="text-sm text-color-text-muted">
                                                Status: {group.status}
                                            </p>
                                        </div>
                                        <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide ${isActive ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <ul className="text-sm space-y-1">
                                            {otherParticipants.map((otherChild: any) => (
                                                <li key={otherChild.id} className="flex flex-col bg-gray-50 p-2 rounded">
                                                    <span className="font-medium">{otherChild.name}</span>
                                                    <span className="text-xs text-color-text-muted">
                                                        Vrtić: {otherChild.current_kindergarten?.name || 'Nepoznato'}
                                                    </span>
                                                    {isActive && otherChild.parent && (
                                                        <div className="mt-1 pt-1 border-t border-gray-200">
                                                            <p className="text-xs font-bold text-teal-700">Kontakt roditelja:</p>
            ...
            <div className="glass-card p-6 rounded-2xl border border-green-100 bg-green-50/30">
                <div className="flex items-center gap-3 mb-4">
                    ...
                    <h3 className="text-lg font-bold text-green-800">Direktna podudaranja</h3>
                </div>
                {matches.length > 0 ? (
                    <div className="grid gap-3">
                        {matches.map(m => (
                            <div key={m.id} className="bg-white p-4 rounded-xl shadow-sm border border-green-100 flex justify-between items-center">
                                <div>
                    ...
                    <p className="text-color-text-muted italic">Nema direktnih podudaranja.</p>
                )}
            </div>
            ...
            <div className="glass-card p-6 rounded-2xl border border-orange-100 bg-orange-50/30">
                <div className="flex items-center gap-3 mb-4">
                    ...
                    <h3 className="text-lg font-bold text-orange-800">Potencijalna kružna podudaranja</h3>
                </div>
                {potentials.length > 0 ? (
                    <div className="text-sm bg-white/50 p-4 rounded-xl max-h-48 overflow-y-auto">
                        <pre className="whitespace-pre-wrap">{JSON.stringify(potentials, null, 2)}</pre>
                    </div>
                ) : (
                    <p className="text-color-text-muted italic">Nema potencijalnih podudaranja.</p>
                )}
            </div>
        </div>
    );
...
export default function Dashboard() {
    ...
    return (
        <div className="min-h-screen relative overflow-hidden bg-gray-50 pt-20">
            ...
            <div className="absolute top-0 right-0 w-96 h-96 bg-purple-200 rounded-full mix-blend-multiply filter blur-3xl opacity-30 animate-float pointer-events-none" style={{ animationDelay: '1s' }}></div>
            <div className="absolute bottom-0 left-1/2 w-96 h-96 bg-pink-200 rounded-full mix-blend-multiply filter blur-3xl opacity-30 animate-float pointer-events-none" style={{ animationDelay: '2s' }}></div>
                        ...
                        <div className="glass-card p-6 rounded-3xl sticky top-24">
                            ...
                            <div className="space-y-4">
                                <div className="p-4 bg-white/50 rounded-xl">
                                    <p className="text-xs uppercase tracking-wider text-color-text-muted font-bold mb-1">Ime i prezime</p>
                                    <p className="font-semibold text-lg">{user.full_name}</p>
                                </div>
                                <div className="p-4 bg-white/50 rounded-xl">
                                    <p className="text-xs uppercase tracking-wider text-color-text-muted font-bold mb-1">Email adresa</p>
                                    <p className="font-semibold text-lg break-all">{user.email}</p>
                                </div>
                            </div>

                            <button className="w-full mt-6 py-3 px-4 bg-red-50 text-red-600 rounded-xl font-semibold hover:bg-red-100 transition-colors flex items-center justify-center gap-2"
                                onClick={() => {
                            ...
                            {user.children.length === 0 ? (
                                <div className="text-center py-20 bg-gray-50/50 rounded-2xl border border-dashed border-gray-300">
                                    ...
                                    <p className="text-color-text-muted max-w-md mx-auto mb-6">
                                        Dodajte podatke o vašoj deci da biste započeli pretragu za razmenu vrtića.
                                    </p>
                                ...
                                <Tabs defaultValue={user.children[0].id} className="w-full">
                                    ...
                                    {user.children.map((child) => (
                                        <TabsContent key={child.id} value={child.id} className="mt-0 focus-visible:outline-none focus-visible:ring-0">
