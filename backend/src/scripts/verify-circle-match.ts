...
async function verifyCircleMatch() {
    try {
        ...
        const kg1 = kgRepo.create({ name: 'KG_Circle_1', city: 'TestCity', address: 'Addr 1' });
        const kg2 = kgRepo.create({ name: 'KG_Circle_2', city: 'TestCity', address: 'Addr 2' });
        const kg3 = kgRepo.create({ name: 'KG_Circle_3', city: 'TestCity', address: 'Addr 3' });
        ...
        await wishlistRepo.save(wishlistRepo.create({ child: childA, target_kindergarten: kg2 }));
        ...
        await wishlistRepo.save(wishlistRepo.create({ child: childB, target_kindergarten: kg3 }));
        ...
        await wishlistRepo.save(wishlistRepo.create({ child: childC, target_kindergarten: kg1 }));
        ...
        const potentialMatches = await matchingService.findPotentialMatches(AgeGroup.MLADJA);

        console.log(`Found ${potentialMatches.length} potential matches.`);

        const circleMatch = potentialMatches.find(m =>
            m.children.length === 3 &&
        ...
        if (circleMatch) {
            ...
            console.log('Children involved:', circleMatch.children.map(c => c.name).join(', '));
        } else {
