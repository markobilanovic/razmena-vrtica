...
async function verifyDirectMatch() {
    try {
        ...
        const kg1 = kgRepo.create({ name: 'KG_Test_A', city: 'TestCity', address: 'Address A' });
        const kg2 = kgRepo.create({ name: 'KG_Test_B', city: 'TestCity', address: 'Address B' });
        ...
        const matchesForA = await matchingService.findDirectMatchesForChild(child1.id);
        console.log('Matches for A:', matchesForA.map(k => k.name));
        ...
        const matchesForB = await matchingService.findDirectMatchesForChild(child2.id);
        console.log('Matches for B:', matchesForB.map(k => k.name));
