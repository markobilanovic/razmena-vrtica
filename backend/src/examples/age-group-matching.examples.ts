...
function exampleCalculateAgeGroup() {
    ...
    console.log(`Birth date: ${birthDate1.toDateString()}, Age Group: ${ageGroup1}`);
    ...
    console.log(`Birth date: ${birthDate2.toDateString()}, Age Group: ${ageGroup2}`);
    ...
    console.log(`Birth date: ${birthDate3.toDateString()}, Age Group: ${ageGroup3}`);
...
async function exampleGetMatchesByAgeGroup(matchingService: MatchingService) {
    const starijaMatches = await matchingService.getMatchesByAgeGroup(AgeGroup.STARIJA);
...
async function exampleValidateMatch(matchingService: MatchingService, matchId: string) {
...
async function exampleRealisticScenario(childService: ChildService, matchingService: MatchingService) {
    ...
    console.log(`${ana.name}: Age group ${ana.group}, at Kindergarten A, wants Kindergarten B`);
    console.log(`${milan.name}: Age group ${milan.group}, at Kindergarten B, wants Kindergarten A`);
    ...
    const potentialMatches = await matchingService.findPotentialMatches(AgeGroup.STARIJA);
...
async function exampleDifferentAgeGroups(childService: ChildService, matchingService: MatchingService) {
